...
Release, failed-acquisition cleanup, and stale recovery first atomically rename the active lock path to an unpredictable diagnostic name, then revalidate the moved token or filesystem identity. A successful revalidation authorizes only removal from the active path: the diagnostic entry is retained, because its name can be concurrently replaced after any owner read or identity check. A mismatch, corrupt metadata, or unknown post-`mkdir` identity never renames the diagnostic back to the canonical path: directory rename can overwrite an existing empty directory on supported filesystems and therefore is not a no-overwrite restore. Instead, the diagnostic remains permanent and the controller fails closed with a typed error that names it. It attempts `mkdir(lockPath, { mode: 0o700 })` to reserve only a missing canonical path; `EEXIST` means a concurrent entry won and is left byte-for-byte and inode-for-inode untouched. An empty reservation is intentionally rejected as corrupt by future acquisition rather than interpreted as stale or successfully initialized. The initial identity read and owner write after successful lock-directory creation share this quarantine path, so even an unknown identity is atomically moved aside without deletion before the reservation attempt. Automated lock operations never call `rm`, `rmdir`, or unlink on a diagnostic entry; inspection and cleanup are reserved for a future explicit, lock-protected maintenance operation.
