---
name: ddd-roadmap
description: Generate phased DDD development roadmaps and the product brief that anchors them. Triggers: "generate roadmap", "plan phases", "create roadmap", "/ddd-roadmap <scope>", "generate brief", "create product brief", "ddd-brief" (merged into this skill). Supports full-project, scoped, and interactive modes. Produces docs/product-brief.md + checkbox-format roadmap for ddd-spec/ddd-develop/ddd-auto consumption.
argument-hint: "[scope description and/or PRD file paths...]"
allowed-tools:
  - Bash(*)
  - Edit
  - Write
...
1. **Scoped roadmap** — `/ddd-roadmap <scope>` generates a roadmap focused on the given feature area or requirement. Arguments that are file paths (start with `/`, `./`, `../`, or end with `.md`/`.txt`) are treated as primary product sources (PRD, design doc) and read in full during Step 1.
...
This skill also owns the **product brief**: Step 2.5 writes/updates `docs/product-brief.md` from the aligned understanding. There is no separate brief skill — requests like "generate product brief" run this skill (a brief-only update can stop after Step 2.5).
...
1. `docs/product-brief.md` — canonical product anchor (written by this skill's Step 2.5 in a previous run). **If found, treat as the highest-authority source.** Its Goals, Non-goals, Key Design Decisions, and Constraints sections directly constrain feature decomposition and phase prioritization. Do not contradict these unless the user explicitly overrides them during Goal Alignment.
2. Project root: `PRD.md`, `PRODUCT.md`, `REQUIREMENTS.md`, `SPEC.md`
3. `docs/` directory: `docs/prd*`, `docs/product*`, `docs/requirements*`, `docs/spec*`, `docs/design*`
4. `docs/specs/`, `docs/requirements/`, `docs/design/` subdirectories
5. `docs/superpowers/specs/` — design specs generated by superpowers brainstorming (treat as PRD-level input)
6. Any `.md` or `.txt` file whose name or H1 heading contains: PRD, product requirement, product spec, functional spec, user story, acceptance criteria, use case
...
5. **Carry everything into Step 2.5** — the aligned understanding (including corrections the user just made) is the input for writing/updating the product brief.
...
## Step 2.5 — Write/Update Product Brief

Distill the aligned understanding into `docs/product-brief.md` — the canonical anchor consumed by ddd-spec (which blocks without it), ddd-develop, and ddd-audit.

**Read `references/product-brief-format.md`** in this skill's directory for the file format, distillation rules (no invention, preserve rationale, flag gaps), and write rules.

- If the file exists: merge new decisions, constraints, and non-goals into it — never overwrite wholesale
- If it doesn't: create it from the Step 1 sources + Step 2 alignment
- **Brief-only invocations** (user asked to "generate/update the product brief" rather than plan): present the brief for review, commit it, and stop here — skip Steps 3-7

Sources feed the brief in priority order: user-provided input (inline text, file paths from arguments) > session context (decisions made in this conversation) > product docs discovered in Step 1 > README.
...
After the user approves the roadmap, offer to generate behavior contracts (specs) for all feature areas. `docs/product-brief.md` is guaranteed to exist at this point (written in Step 2.5), so the spec anchor is in place.
...
```bash
git add docs/product-brief.md docs/roadmap/

# Full-project mode:
git commit -m "docs: add product brief and project roadmap (P0-P3)"
...
- `docs/product-brief.md` from a previous run (highest-priority source; if absent, Step 2 compensates with full Q&A and Step 2.5 creates it)

**Produces artifacts consumed by:**
- **ddd-spec / ddd-develop / ddd-audit** — `docs/product-brief.md` (Step 2.5) anchors spec generation, implementation decisions, and goal-compliance checks
