...
1. `docs/product-brief.md` — canonical product anchor generated by `/ddd-brief`. **If found, treat as the highest-authority source.** Its Goals, Non-goals, Key Design Decisions, and Constraints sections directly constrain feature decomposition and phase prioritization. Do not contradict these unless the user explicitly overrides them during Goal Alignment.
...
5. **Write new decisions back** — If this alignment session produces decisions, constraints, or non-goals that are NOT already captured in `docs/product-brief.md`, append them to product-brief.md before generating the roadmap. This keeps the brief authoritative across sessions. If `docs/product-brief.md` does not exist and significant new decisions were made, suggest the user run `/ddd-brief` to formalize them.
...
**First, check `docs/product-brief.md`:**
- If the file **exists**: proceed with the spec generation offer below.
- If the file **does not exist**: present this instead:

  ```
  Roadmap approved.

  Before generating specs, a product brief is needed to anchor specs to the
  original product intent (prevents specs from drifting when expanded from
  the roadmap alone).

  Run /ddd-brief first to generate docs/product-brief.md, then run /ddd-spec.

  Or skip specs now and run /ddd-spec manually after creating the brief.
  ```
  Then continue to Step 7.

**If product-brief.md exists**, present:
...
- `docs/product-brief.md` produced by **ddd-brief** (highest-priority source; if absent, Step 2 compensates with full Q&A)
