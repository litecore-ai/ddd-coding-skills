---
name: ddd-spec
description: Use when generating structured behavior contracts (specs) for feature areas from a roadmap - triggers on "generate spec", "create spec", "ddd-spec", "/ddd-spec", "/ddd-spec P0.1", "/ddd-spec P0.1 - P0.3", "/ddd-spec P0". Requires docs/product-brief.md (generated by /ddd-roadmap). Produces acceptance-criteria-driven spec documents that anchor ddd-develop plans to testable contracts.
...
digraph ddd_spec {
  ...
  brief_check [label="docs/product-brief.md\nexists?" shape=diamond];
  blocked [label="BLOCKED\nReport: run /ddd-roadmap first\n(it writes the brief)"];
  ...
  brief_check -> blocked [label="no"];
  brief_check -> input [label="yes"];
  input -> parse;
  parse -> has_scope;
...
## Pre-check -- Product Brief Required

Before doing anything else, verify that `docs/product-brief.md` exists.
...
ddd-spec requires a product brief to anchor specs to the original product intent.
Without it, specs are generated by expanding the roadmap alone, which causes drift.

Run /ddd-roadmap first — its Goal Alignment step (2.5) writes docs/product-brief.md.
To refresh only the brief without re-planning: /ddd-roadmap update the product brief.
```

Do not proceed past this point until the file exists.
...
Priority order — higher sources override lower sources on conflicts.

1. **`docs/product-brief.md`** (highest priority): the canonical product anchor. Read in full. Extract:
   - Product vision, target users, and goals
   - Non-goals — use these to constrain spec scope; do not generate ACs for out-of-scope items
   - Key design decisions and their rationale — these constrain data models and API contracts
   - Constraints (performance, compliance, integration, technology)
   - Open questions — flag these explicitly in generated specs

2. **Roadmap files**: `docs/roadmap/P{n}-*.md` for each phase in scope. Extract:
...
3. **CLAUDE.md**: project conventions, DDD layer structure, naming patterns, tech stack

4. **Existing code**: scan `src/` (or equivalent) for modules related to the feature area. Extract:
   - Existing domain models, value objects, entities
   - Existing API endpoints and their signatures
   - Existing test patterns and conventions
   - Database schema or migration files

5. **Existing specs**: check `docs/specs/` for specs that already cover the feature areas in scope
...
**Consumes (required):**
- `docs/product-brief.md` generated by **ddd-roadmap** (Step 2.5) — blocks if missing
...
**Pipeline position:**
```
ddd-roadmap (brief + roadmap) ──→ ddd-spec ──→ ddd-develop ──→ ddd-audit
                                                    ↑
                                                 ddd-auto
```
